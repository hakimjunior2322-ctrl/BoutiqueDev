...
export default function NeonPortfolio() {
  ...
  const handleChatSend = () => {
    ...
    setChatInput('')
    setTimeout(() => {
      setChatMessages(prev => [...prev, { id: prev.length + 1, sender: 'bot', text: 'Merci pour ta question ! Je passe ta demande à notre équipe.' }])
    }, 500)
  }
    ...
    <div className={isTerminalMode ? 'bg-gray-900 text-green-400 font-mono' : 'bg-black text-white'}>
      ...
      <nav className="fixed top-0 left-0 right-0 z-40 bg-black/80 backdrop-blur-md border-b border-pink-500/20">
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-16 md:h-20 flex items-center justify-between">
          <h1 className="text-xl md:text-2xl font-black bg-gradient-to-r from-pink-400 to-purple-400 bg-clip-text text-transparent">JuDev</h1>
          <div className="flex items-center gap-1 md:gap-4">
            ...
            <div className="lg:hidden flex gap-1">
              {navItems.map((item) => (
                <button key={item.id} onClick={() => setCurrentPage(item.id)} className={`px-2 py-1 text-xs font-semibold rounded transition-all ${currentPage === item.id ? 'bg-pink-500/30 text-pink-300' : 'text-gray-400'}`}>
                  {item.label.split(' ')[0]}
                </button>
              ))}
            </div>
            <button onClick={() => setIsTerminalMode(!isTerminalMode)} className="px-3 md:px-4 py-2 text-xs md:text-sm border border-pink-500/50 text-pink-400 rounded-lg hover:bg-pink-500/10 transition font-mono">
      ...
      {/* PAGE HOME */}
      {currentPage === 'home' && (
        <div className="min-h-screen pt-16 md:pt-20 bg-gradient-to-b from-black via-purple-950/20 to-black flex items-center justify-center">
      ...
      {/* PAGE JUDEV */}
      {currentPage === 'judev' && (
        <div className="min-h-screen pt-20 md:pt-32 pb-16 md:pb-20 bg-black">
          <div className="max-w-7xl mx-auto px-4 md:px-6">
            ...
            {selectedMedia && (
              <div onClick={() => setSelectedMedia(null)} className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4">
          ...
          <div className="max-w-7xl mx-auto px-4 md:px-6">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-12 gap-6">
              <div>
                ...
                <p className="text-gray-400 text-lg md:text-xl">Nos clients satisfaits partagent leur expérience</p>
              </div>
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setShowAvisForm(!showAvisForm)} className="px-6 py-3 bg-gradient-to-r from-pink-500 to-purple-500 text-white rounded-lg font-semibold hover:from-pink-600 hover:to-purple-600 transition w-full md:w-auto">
                + Laisser un avis
              </motion.button>
            </div>
              ...
              <div className="mb-12 bg-gradient-to-br from-pink-950/20 to-purple-950/20 border border-pink-500/20 rounded-xl p-6 md:p-12">
                ...
                <form onSubmit={handleAvisSubmit} className="space-y-6">
                  <div>
                    ...
                    <input type="text" placeholder="Jean Dupont" value={avisFormData.name} onChange={(e) => setAvisFormData({ ...avisFormData, name: e.target.value })} className="w-full bg-black/50 border border-pink-500/30 rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-pink-500 transition" required />
                  </div>
                  <div>
                    <label className="block text-white font-semibold mb-2">Votre entreprise / Secteur</label>
                    <input type="text" placeholder="Ex: Agence web, E-commerce, Startup..." value={avisFormData.business} onChange={(e) => setAvisFormData({ ...avisFormData, business: e.target.value })} className="w-full bg-black/50 border border-pink-500/30 rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-pink-500 transition" />
                  </div>
                  <div>
                    <label className="block text-white font-semibold mb-2">Email *</label>
                    <input type="email" placeholder="votre@email.com" value={avisFormData.email} onChange={(e) => setAvisFormData({ ...avisFormData, email: e.target.value })} className="w-full bg-black/50 border border-pink-500/30 rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-pink-500 transition" required />
                  </div>
                  <div>
                    <label className="block text-white font-semibold mb-4">Votre note *</label>
                    <div className="flex gap-2">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <button key={star} type="button" onClick={() => setAvisFormData({ ...avisFormData, rating: star })} className={`text-5xl transition-all ${star <= avisFormData.rating ? 'text-yellow-400' : 'text-gray-500'}`}>
                          ⭐
                        </button>
                      ))}
                    </div>
                  ...
                  <div>
                    ...
                    <textarea placeholder="Partagez votre expérience..." rows="6" value={avisFormData.text} onChange={(e) => setAvisFormData({ ...avisFormData, text: e.target.value })} className="w-full bg-black/50 border border-pink-500/30 rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-pink-500 transition resize-none" required />
                  </div>
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} type="submit" disabled={avisSubmitting} className="w-full py-3 bg-gradient-to-r from-pink-500 to-purple-500 text-white rounded-lg font-semibold hover:from-pink-600 hover:to-purple-600 transition disabled:opacity-50">
                    {avisSubmitting ? 'Envoi...' : 'Publier mon avis'}
                  </motion.button>
                </form>
                <p className="text-gray-400 text-sm text-center mt-8">Les avis sont modérés avant publication. Merci de votre compréhension !</p>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
              {tarifs.length === 0 ? (
                ...
                <motion.div key={plan.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className={`relative rounded-xl p-6 md:p-8 border transition-all ${plan.highlight ? 'bg-gradient-to-br from-blue-950/40 to-cyan-950/40 border-blue-500/50 md:scale-105' : 'bg-gradient-to-br from-blue-950/10 to-cyan-950/10 border-blue-500/20 hover:border-blue-500/50'}`}>
                  {plan.highlight && (
                    <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-blue-500 to-cyan-500 px-4 py-1 rounded-full text-sm font-semibold text-white">RECOMMANDÉ</div>
                  )}
      ...
      {/* PAGE CONTACT */}
      {currentPage === 'contact' && (
        <div className="min-h-screen pt-20 md:pt-32 pb-16 md:pb-20 bg-black flex items-center justify-center">
          <div className="max-w-4xl mx-auto px-4 md:px-6 w-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 mb-8 md:mb-12">
              ...
              <div className="bg-gradient-to-br from-blue-950/20 to-cyan-950/20 border border-blue-500/20 rounded-xl p-6 md:p-8">
                ...
                <button onClick={() => setShowChat(!showChat)} className="text-blue-300 hover:text-blue-200 transition">
            ...
            {showChat && (
              <div className="bg-gradient-to-br from-blue-950/30 to-cyan-950/30 border border-blue-500/30 rounded-xl p-4 md:p-6 mb-8">
                ...
                <div className="flex gap-2">
                  <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleChatSend()} placeholder="Écris ton message..." className="flex-1 bg-black/50 border border-blue-500/30 rounded-lg px-4 py-2 text-white placeholder-gray-500 focus:outline-none focus:border-blue-500" />
                  <button onClick={handleChatSend} className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition">Envoyer</button>
                </div>
              </div>
            )}
